# api/analisador.py - VERSÃO DE PRODUÇÃO FINAL
...
# Se a chave não for encontrada, o programa quebra com uma mensagem clara.
if not API_KEY:
    raise ValueError("ERRO CRÍTICO: API_KEY não configurada no Vercel.")

# Se a linha acima passar, a chave FOI encontrada.
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if not pdf_url:
                raise ValueError("URL do PDF não fornecida.")

            headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
            response = requests.get(pdf_url, headers=headers, timeout=90, allow_redirects=True) # Aumentei o timeout
            ...
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({'erro': f'Ocorreu um erro no servidor: {str(e)}'}).encode('utf-8'))
